...
module.exports = app => {
  ...
  app.post("/roll/:sides/:roll_cnt/:user_id", (req, res) => {
    ...
    sql.query("SELECT * FROM scores where user_id=?", [req.params.user_id], (err, result) => {
      if (!err)
      {
        const datetime = new Date().toJSON().slice(0, 19).replace('T', ' ')
        var today_start = new Date();
        today_start.setHours(0,0,0,0);
        let today_start_norm = today_start.toJSON().slice(0, 19).replace('T', ' ');

        if((typeof result === 'undefined') || result.length === 0 || (sum > result[0].score))
        {
          // This is a new highscore for the player all time
          highscore = true;          
          day_highscore = true;
          sql.query("REPLACE INTO scores (user_id, score, day_score_date, day_score) VALUES(?,?,?,?)", 
                [req.params.user_id, sum, datetime, sum], (err, result) => {
            if (err) {
              //console.log(err);
            }
          });
        ...
        {
          let day_score_norm = result[0].day_score_date.toJSON().slice(0, 19).replace('T', ' ');
          if((sum > result[0].day_score) || (day_score_norm < today_start_norm))
          {
            // This is a new highscore for the player on this day
            day_highscore = true;
            sql.query("REPLACE INTO scores (user_id, score, day_score_date, day_score) VALUES(?,?,?,?)", 
                  [req.params.user_id, result[0].score, datetime, sum], (err, result) => {
              if (err) {
                //console.log(err);
              }
            });
  ...
  app.get("/user/:user_id", (req, res) => {
    sql.query("SELECT * FROM users where user_id=? ", [req.params.user_id], (err, result) => {
      if (err) {
        console.log(err);
        res.status(500);
        res.json({err_msg: "Server Error"});
  ...
  app.get("/highscores/today", (req, res) => {
    ...
    sql.query("SELECT users.username, scores.day_score FROM users \
               INNER JOIN scores ON users.user_id=scores.user_id \
               WHERE scores.day_score_date > ? \
               ORDER BY scores.day_score DESC", [today_start_sql], (err, result) => {
      if (!err) {
        res.json(result)
      }
    });
  ...
  app.get("/highscore/today/:user_id", (req, res) => {
    ...
    today_start.setHours(0,0,0,0);
    let today_start_sql = today_start.toJSON().slice(0, 19).replace('T', ' ');
    sql.query("SELECT users.username, scores.day_score, scores.day_score_date FROM users \
               INNER JOIN scores ON users.user_id=scores.user_id \
               WHERE users.user_id=? AND scores.day_score_date > ?", [req.params.user_id, today_start_sql], (err, result) => {
      if (!err) {
        var day_score = 0;
        if(typeof result !== 'undefined' && result.length > 0)
        {
